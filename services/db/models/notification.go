...
func (o *Notification) UserG(mods ...qm.QueryMod) userProfileQuery {
	return o.User(boil.GetDB(), mods...)
}

// User pointed to by the foreign key.
func (o *Notification) User(exec boil.Executor, mods ...qm.QueryMod) userProfileQuery {
...
func (notificationL) LoadUser(e boil.Executor, singular bool, maybeNotification interface{}) error {
...
// SetUserP of the notification to the related item.
// Sets o.R.User to related.
// Adds o to related.R.UserNotifications.
// Panics on error.
func (o *Notification) SetUserP(exec boil.Executor, insert bool, related *UserProfile) {
	if err := o.SetUser(exec, insert, related); err != nil {
		panic(boil.WrapErr(err))
	}
}

// SetUserGP of the notification to the related item.
// Sets o.R.User to related.
// Adds o to related.R.UserNotifications.
// Uses the global database handle and panics on error.
func (o *Notification) SetUserGP(insert bool, related *UserProfile) {
...
func (o *Notification) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"notification\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
...
func (o *Notification) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"notification\"", updateOnConflict, ret, update, conflict, insert)
