...
func (c *Configurator) submitTransaction(transaction string) error {
	localLog := c.log.WithField("tx", transaction)
	localLog.Info("Submitting transaction")

	_, err := c.Horizon.SubmitTransaction(transaction)
	if err != nil {
...
func (c *Configurator) signAndSubmitTransaction(transaction string, seed string) error {
	localLog := c.log.WithField("tx", transaction)

	localLog.Debug("Decoding transaction")
	tx, err := c.decodeFromBase64(transaction)
	if err != nil {
		return err
	}

	localLog.Debug("Signing transaction")
	s := build.Sign{Seed: seed}
	err = s.MutateTransactionEnvelope(tx)
	if err != nil {
		return err
	}
